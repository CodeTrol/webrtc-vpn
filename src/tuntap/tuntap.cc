...
bool Tuntap::construct(Handle<Object> main_obj, std::string &error) {
	...
	ifr.ifr_flags |= IFF_NO_PI | IFF_ONE_QUEUE;
	...
	struct sockaddr_in *saddr = (struct sockaddr_in *)&rt.rt_gateway;
	saddr->sin_family = AF_INET;
	saddr->sin_addr.s_addr = 0;
	saddr = (struct sockaddr_in*)&rt.rt_dst;
	saddr->sin_family = AF_INET;
	saddr->sin_addr.s_addr = inet_addr((addr.substr(0, addr.find_last_of(".")) + ".0").c_str());
	saddr = (struct sockaddr_in*)&rt.rt_genmask;
	saddr->sin_family = AF_INET;
	saddr->sin_addr.s_addr = inet_addr(mask.c_str());
	strcpy(c_itf_name, this->itf_name.c_str());
	rt.rt_dev = c_itf_name;
	rt.rt_flags = RTF_UP;
	rt.rt_metric = 10;

	MK_IOCTL(tun_sock, SIOCADDRT, &rt)
	...
	this->read_buff = new unsigned char[this->mtu + 14];
...
void Tuntap::do_read() {
	...
	ret = read(this->fd, this->read_buff, this->mtu + 14);
